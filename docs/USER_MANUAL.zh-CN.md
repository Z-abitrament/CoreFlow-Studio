...
- K Factor 和 Repeatability 弹窗中的 `PC simulate flow segment` 是实验室联调用的测试开关，适用于已连接真实从机但暂时没有真实流量的情况。勾选后，软件仍会正常读取从机的 `flow_rate` 和累计量变量以验证通信，但流程判断使用 PC 端给出的非零/回零状态，计算使用 `PC Sim Delta m` 作为本次累计量变化。该功能不会把流量变量写入从机；保存的结果会标记 `flow_rate_source=pc_simulated`。
