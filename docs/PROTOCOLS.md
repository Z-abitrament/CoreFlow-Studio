...
- Error/repeatability: in Three Flow Ranges mode, run three configured target-flow ranges with three trials per range. Each trial captures accumulated mass before/after one reusable non-zero-to-zero flow segment, records the third-second instantaneous flow `v1` and mean flow `v_mean`, accepts the operator's standard-scale mass, and calculates trial percent error immediately after the operator saves that trial. Per-range repeatability standard deviation is calculated as soon as that range's third trial is saved, and the completed nine-trial summary is stored when history recording is enabled. Single Flow Range mode uses one configured target-flow range, allows the operator to append more trials at any time, refreshes the current error/repeatability summary after each saved trial, and stores the current summary on explicit `Save Summary`. Advanced mode is reserved for future multi-run or fitting logic.
