...
## Modbus Zero Monitor Fake Scenarios

The M16 zero monitor is exercised first through the Modbus fake transport, not
by inventing a production transmitter model. Committed fixtures live under
`tests/fixtures/modbus_zero_monitor/` so CI does not depend on the external
firmware checkout. `firmware_snapshot_vectors.json` records source paths,
content hashes, the observed firmware HEAD, literal 18-word blocks, and expected
decoded values derived from the approved `Krohne_prj` PC tests.

`host_scenarios.json` keeps host-only fault and analysis cases separate and
marks synthetic acceptance limits as test-only. Together the fixtures cover:

- Startup before the 600 ms live window is ready.
- Stable, noisy, drifting, stepped, and spiked live-zero sequences.
- Invalid count, cleared data-valid bit, NaN/Inf payload, and internal-error bit.
- Base/live-ready inconsistency, zero-calibration-active intervals, and nonzero
  reserved status bits with deterministic segment recovery.
- Matching and torn begin/end sequences.
- Duplicate, missing, and wrapping 16-bit sequences.
- Wrapping and unexpectedly regressing 32-bit device ticks.
- Timeout, exception response, delayed response, poll overrun, disconnect, and
  reconnect into a new continuous segment.
- Four supported multi-register byte/word-order combinations.

Fixtures must assert that each poll is one merged read and that no write method
is called. Their numeric values and thresholds are labeled test inputs; they do
not define production stability limits or prove a physical zero-flow condition.
Golden raw words are literals and must not be generated by the encoder/decoder
under test. Firmware source or hash changes require an explicit reviewed fixture
update; test execution never refreshes expected values.
Recorded real-device snapshot CSV files may later be replayed through the pure
analysis path, preserving their original device time, sequence gaps, register
map, and zero-flow-context metadata.
