...
This repository has completed M12 Windows packaging foundation. It contains the canonical documentation harness, minimal Python package, entry point, dependency metadata, smoke tests, `FlowmeterDevice`, device data models, workflow status models, storage artifact models, deterministic simulator scenarios, `SimulatedFlowmeterDevice`, a multi-device simulator manager, Modbus register-map models, encoding/decoding helpers, a fake-testable transport abstraction, `ModbusRtuFlowmeterDevice`, SQLite schema initialization, metadata repositories, artifact file storage, artifact integrity checks, calibration preview models, a placeholder calibration calculator, `WriteGuardService`, a headless calibration preview workflow, a headless automated factory test workflow, initial error/stability calculation modules, an application runtime facade, a PySide6 main window for simulator-backed connection, live readings, calibration preview, factory test, flexible experiment, run history, and result inspection, report/export generation for plain-text operator reports, metrics CSV, measurement CSV, and export manifests, experiment definition models, signal-processing module interfaces, no-op fixture/ML placeholders, a simulator-backed experiment workflow, placeholder Modbus register-map template helpers, serial-port discovery abstraction, a read-only hardware acceptance runner with write-guard dry-run checks, PyInstaller packaging configuration, a windowed Windows UI executable, a console diagnostics executable, Windows build script, build metadata stamping, a user data directory convention, and English/Chinese user manuals. It does not yet contain production calibration formulas, calibration apply workflow, customer-specific report templates, real fixture drivers, real ML model execution, replay-file support, signed installer/MSI, automatic real-hardware UI enablement, or armed hardware write workflows.
