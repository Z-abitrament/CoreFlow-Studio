...
This repository has completed M9 reporting and export foundation. It contains the canonical documentation harness, minimal Python package, entry point, dependency metadata, smoke tests, `FlowmeterDevice`, device data models, workflow status models, storage artifact models, deterministic simulator scenarios, `SimulatedFlowmeterDevice`, a multi-device simulator manager, Modbus register-map models, encoding/decoding helpers, a fake-testable transport abstraction, `ModbusRtuFlowmeterDevice`, SQLite schema initialization, metadata repositories, artifact file storage, artifact integrity checks, calibration preview models, a placeholder calibration calculator, `WriteGuardService`, a headless calibration preview workflow, a headless automated factory test workflow, initial error/stability calculation modules, an application runtime facade, a PySide6 main window for simulator-backed connection, live readings, calibration preview, factory test, run history, and result inspection, plus report/export generation for plain-text operator reports, metrics CSV, measurement CSV, and export manifests. It does not yet contain production calibration formulas, calibration apply workflow, customer-specific report templates, hardware acceptance, replay-file support, packaging, or real hardware UI enablement.
